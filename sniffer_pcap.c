...
void got_packet(u_char *args, const struct pcap_pkthdr *header, const u_char *packet)
{
    ...
    if ((tcp->tcp_flags & TH_SYN) && !(tcp->tcp_flags & TH_ACK)) {
        printf("[Forensic Note] SYN without ACK. This may be a connection attempt or scan indicator.\n");
    }
    if (tcp->tcp_flags & TH_RST) {
        printf("[Forensic Note] RST observed. This can appear in rejected connections or scan responses.\n");
    }
